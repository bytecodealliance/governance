# Proposal to Create regalloc3

This document is a proposal to create regalloc3 as a formal Hosted Project under the auspices of the TSC of the Bytecode Alliance, as specified in the TSC’s charter.

Proposing the adoption of regalloc3 as a Bytecode Alliance hosted project.

Current Repository URL: https://github.com/Amanieu/regalloc3 (for existing projects being proposed for adoption)

Proposed Repository URL: https://github.com/bytecodealliance/regalloc3 (URL of the repository to be created within the Bytecode Alliance GitHub organization)

Regalloc3 is a new embeddable register allocator implementation designed as a successor to [regalloc2](https://github.com/bytecodealliance/regalloc2). In [benchmarks](https://bytecodealliance.zulipchat.com/#narrow/channel/217117-cranelift/topic/regalloc3.20benchmarks/near/527201621) it was shown to produce faster code than regalloc2 while decreasing compilation time. Regalloc3 also supports additional features such as rematerialization and fixed stack slots which, once leveraged by Cranelift, could result in further increases to the performance of generated code.
...
Regalloc3 provides a reusable component for building compilers, which is in line with the Bytecode Alliance Mission.
...
The main branch is currently being [reviewed](https://github.com/Amanieu/regalloc3/pull/2). Once the review is complete, the project will switch to a pull request model where all changes go through a PR which is reviewed by another contributor.
...
This will be added once the project is accepted by the Bytecode Alliance.
...
The project runs [CI tests](https://github.com/Amanieu/regalloc3/blob/main/.github/workflows/rust.yml) on all pull requests, including some fuzz testing (capped at 60 seconds).
...
A `CONTRIBUTING.md` file will be added pointing to the Bytecode Alliance zulip as a place for discussions once this project is accepted.
...
This project is a high-quality, well-tested and performant register allocator designed as a reusable component, which is in line with the Bytecode Alliance Operational Principles.
...
The project is [currently licensed](https://github.com/Amanieu/regalloc3/blob/main/Cargo.toml) under `MIT OR Apache-2.0`. This can be changed to Apache-2.0 WITH LLVM-exception when the project is accepted.
...
The project has a [`README.md`](https://github.com/Amanieu/regalloc3/blob/main/README.md). The Bytecode Alliance line will be added once the project is accepted.
...
The project has no releases yet, but the plan is to implement automated releases using [release-plz](https://release-plz.dev/). This will automate the release process to consist of simply merging a PR.
...
This project will follow the same security process as Cranelift, which will be the main user of the register allocator.
...
The project has no releases yet but will follow the standard semantic versioning of the Rust ecosystem.
...
The project has no secrets. In the future release-plz will use secrets for automated releases which will be kept in the GitHub repo secrets.
...
This project has minimal dependencies and will add `cargo vet` following Wasmtime's vetting once accepted.
...
The project currently only has a single contributor, but is open to adding more maintainers.
...
The project is currently hosted on Github. It will be moved to the Bytecode Alliance once accepted.
...
The project has no changelog since it has no published releases yet. The plan is to have changelogs be automatically generated by release-plz as part of the automated release process.
...
The project has several [fuzz targets](https://github.com/Amanieu/regalloc3/tree/main/fuzz/fuzz_targets) which exercise almost all of the code paths in the register allocator. These are not currently being continously run.
...
The project has extensive [API documentation](https://github.com/Amanieu/regalloc3/blob/main/src/lib.rs#L1) and an in-depth [design document](https://github.com/Amanieu/regalloc3/blob/main/DESIGN.md) that gives an overview of the implementation details of the register alloctor.
...
The project uses Github as its issue tracker.
...
The project will adopt the Bytecode Alliance RFC process after it is accepted.
...
This project will be used in production as part of Cranelift, and eventually will become the default register allocator for it.
...
This project will be covered by the existing weekly Cranelift meeting.
...
This project does not use any unsafe code, so there is no need for tools such as `valgrind` or `miri`. It does run Clippy in CI for additional static analysis.

One dependency of the project, `brie-tree`, uses extensive unsafe code but is tested with both fuzzing and `miri` in CI.
