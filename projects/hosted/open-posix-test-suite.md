...
This project aids the development of `wasi-libc`, and therefore is in alignment with the Bytecode Alliance mission.
...
This will be done under the purview of `wasi-libc`.
...
This falls under the "crucial upstream dependencies" clause.
...
The project will be hosted on GitHub under https://github.com/BytecodeAlliance/open-posix-test-suite.
...
GitHub issues would be used for tracking issues, in the same way as `wasi-libc`.
...
This would fall under the organisation of `wasi-libc`.
...
This test suite is also used by the [Emscripten project](https://github.com/emscripten-core/posixtestsuite). There is evidence of its use for [GNU Hurd](https://www.gnu.org/software/hurd/open_issues/open_posix_test_suite.html) and the [YOCTO Linux project](https://wiki.yoctoproject.org/wiki/Posix_result).
...
Sanitizers and code analysis could be used in conjuction with this test suite in hardening `wasi-libc`.
